"""repair the system full-name rules and add legal full-name support
...
SYSTEM_RULES = (
    ("identity.full_name", "Full name", ["full", "name"]),
    ("identity.legal_full_name", "Legal full name", ["legal", "full", "name"]),
)
...
def downgrade() -> None:
    ...
    conn.execute(
        sa.text(
            ...
            "WHERE user_id IS NULL AND core_field_key = 'identity.legal_full_name' "
            "AND normalized_alias = 'legal full name' AND scene = 'generic'"
        )
    )
