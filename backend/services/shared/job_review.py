...
Experience 和 Projects 都是同等有效的候选人证据。若某个 Project 是重要 JD 要求最直接或最强的证据，应优先保留；不得仅因为类似技术已经出现在工作经历中就删除该 Project。优先级为：事实准确性 > JD 相关性 > 证据强度 > 信息密度 > 节省篇幅。
...
Project 和 Experience 都是同等有效的候选人证据。项目可补充或提供工作经历也涉及的重要 JD 证据；若项目是某项关键要求最直接、最强或唯一的证据，应优先保留，不得仅因类似技术已出现在工作经历中而删除。优先选择高度相关、能展示从零构建、开源或 side project、特定产品能力（例如 map-based software）、重要技术、专业能力或成果的项目。
