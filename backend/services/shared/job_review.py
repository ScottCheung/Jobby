...
不得用合理推断补齐未明确提供的经历；合理 ≠ 有证据。例如，end-to-end ownership 不代表做过 customer discovery，cross-functional collaboration 不代表有 customer communication experience。JD 只能决定什么重要，不能决定候选人做过什么。
...
若某个 bullet 或 Project 是重要 JD 要求最强或唯一的真实证据，应优先保留。不得因为证据来自 Project 就自动降权，也不得仅因为类似技术已在其他经历出现就删除。

优先保留具体成果、ownership、问题解决、可靠性、性能、成本、业务或用户影响。减少重复、泛化职责和低相关实现细节。

直接根据以上规则生成最终 JSON，不需要描述或输出分析过程。
"""
...
保留能为重要 JD 要求提供强证据的项目，尤其是能展示从零构建、独立 ownership、特定产品/领域能力或重要技术成果的项目。

每个项目通常保留 1–3 条 description；高度相关时最多 4 条。删除重复或低价值内容，不得虚构项目、技术、职责或结果。
"""
...
生成 3 段、仅含正文的求职信：不得包含称谓、日期、候选人署名、Sincerely 或其他结语。输出语言由 JD 的主要语言决定；JD 主要为英文时，cover_letter 必须全英文，不得输出中文。系统提示或源简历的语言不得改变这一规则。英文控制在约 180–240 词，中文控制在约 350–450 字。
