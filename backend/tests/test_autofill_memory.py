...
def test_platform_memory_is_scoped_to_ats_and_scene() -> None:
    ...
    assert platform_mapping_scene("micro1", "job application") == "ats_micro1__job_application"
...
def test_platform_memory_falls_back_to_existing_generic_rules() -> None:
    ...
    assert fallback_mapping_scenes("micro1", "job application") == (
        "ats_micro1__job_application",
        "job_application",
    )


def test_new_ats_platforms_use_identifier_hints_for_autofill() -> None:
    for platform in ("icims", "successfactors", "oracle", "workable", "bamboohr", "jora", "micro1"):
