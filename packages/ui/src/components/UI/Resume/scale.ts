...
export function scaleResumeTemplate(
  ...
  return {
    ...
    spacing: {
      ...
      bulletIndent: config.spacing.bulletIndent
        ? scaled(config.spacing.bulletIndent)
        : undefined,
...
export function clampSmartScale(config: ResumeTemplateConfig, scale: number) {
  return Math.min(
