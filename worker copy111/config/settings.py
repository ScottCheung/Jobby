...
safe_mode = True                    # True or False, Note: True or False are case-sensitive. Uses a dedicated profile so LinkedIn login persists between runs.
...
# Run in undetected mode to bypass anti-bot protections. On Apple Silicon Mac, install Rosetta 2 for best results:
...
# >>>>>>>>>>> Question cache & application JSON logs <<<<<<<<<<<

data_folder_path = "data/"
question_cache_file = "data/question_cache.json"
applications_json_file = "data/applications_history.json"
question_similarity_threshold = 0.85   # Fuzzy match threshold between 0 and 1
learn_from_manual_answers = True       # Save answers you fill during manual pause

# >>>>>>>>>>> API data layer <<<<<<<<<<<

# Use the local API service as the main data layer for worker reads/writes.
use_api_data_layer = True
api_base_url = "http://127.0.0.1:8000"
api_timeout_seconds = 5
api_fallback_to_files = False
