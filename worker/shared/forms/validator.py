...
def validate_custom_questions() -> None | ValueError | TypeError:
    '''
    Validates all variables in the `/config/custom_questions.py` file.
    '''
    global __validation_file_path
    __validation_file_path = "config/custom_questions.py"

    if not isinstance(custom_questions, list):
        raise TypeError(f'Invalid input for custom_questions. Expecting a List!')
...
def validate_settings() -> None | ValueError | TypeError:
    ...
    check_string(data_folder_path, "data_folder_path", min_length=1)
    check_string(question_cache_file, "question_cache_file", min_length=1)
    check_string(applications_json_file, "applications_json_file", min_length=1)
    if not 0 < question_similarity_threshold <= 1:
        raise ValueError(f'The variable "question_similarity_threshold" in "{__validation_file_path}" must be between 0 and 1!')
    check_boolean(learn_from_manual_answers, "learn_from_manual_answers")
