...
from shared_services.browser import chrome as open_chrome
from shared_services.persistence import QuestionCache
...
from shared_services.utils.helpers import critical_error_log, print_lg
...
from seekBot.services import singleApply
from seekBot.services.extractor import extract_seek_job_details
...
def _build_seek_apply_config(runtime_globals: dict) -> dict:
    resume_path = os.path.abspath(
        os.path.expanduser(str(runtime_globals.get("default_resume_path") or get_runtime_value("default_resume_path", "") or ""))
    )
    return {
        "first_name": str(runtime_globals.get("first_name") or "").strip(),
        "last_name": str(runtime_globals.get("last_name") or "").strip(),
        "email": str(runtime_globals.get("email") or runtime_globals.get("username") or "").strip(),
        "phone": str(runtime_globals.get("phone_number") or "").strip(),
        "address": " ".join(
            part
            for part in [
                str(runtime_globals.get("street") or "").strip(),
                str(runtime_globals.get("current_city") or "").strip(),
                str(runtime_globals.get("state") or "").strip(),
                str(runtime_globals.get("zipcode") or "").strip(),
            ]
            if part
        ),
        ...
        "cover_letter": str(runtime_globals.get("cover_letter") or "").strip(),
        "salary": str(runtime_globals.get("desired_salary") or "").strip(),
        "notice": str(runtime_globals.get("notice_period") or "").strip(),
...
def _seek_search_url(term: str) -> str:
    ...
    search_location = str(get_runtime_value("search_location", "") or "")
...
def _extract_from_card(card) -> dict | None:
    ...
    return {
        ...
        "work_location": location or str(get_runtime_value("search_location", "Australia") or "Australia"),
...
def _refine_details_from_page(details: dict) -> dict:
    ...
    for selector, key in [(title_selectors, "title"), (company_selectors, "company"), (location_selectors, "work_location")]:
        for css in selector:
            try:
                text = (
                    _safe_title(driver.find_element(By.CSS_SELECTOR, css).text)
                    if key == "title"
                    else _clean_text(driver.find_element(By.CSS_SELECTOR, css).text)
                )
                if text:
                    refined[key] = text
                    break
...
def _skip_reason(details: dict) -> str | None:
    haystack = " ".join(str(details.get(key) or "") for key in ["title", "company", "job_description"]).lower()
...
def _run_quick_apply(details: dict, runtime_globals: dict) -> dict:
    ...
    result = singleApply.apply_to_job(
        job_url,
        config=_build_seek_apply_config(runtime_globals),
        job_context=details,
    )
    ...
    if "message" in result:
        status_callback(str(result["message"]))
    return result
...
def apply_to_seek_jobs(terms: list[str], runtime_globals: dict) -> None:
        ...
        for card in cards:
            ...
            try:
                ...
                if not _has_quick_apply():
                    ...
                    sync_stats_to_status()
                    bot_status(
                        f'Reviewed "{details["title"]}" at {details["company"]}", but it is not SEEK Quick Apply.',
                        status="running",
                    )
                    continue

                bot_status(f'Quick Apply found. Handing job {details["job_id"]} to the SEEK apply bot...', status="running")
                result = _run_quick_apply(details, runtime_globals)
...
def run_seek_flow() -> None:
    ...
    try:
        ...
        apply_api_worker_config(runtime_globals)
        ...
        apply_to_seek_jobs(terms, runtime_globals)
