...
from shared_services.forms.clickers_and_finders import find_by_class
...
def get_job_description(
    ...
    try:
        ...
        for word in list(get_runtime_value("bad_words", []) or []):
            if word.lower() in job_description_low:
                skip_message = f'\n{job_description}\n\nContains bad word "{word}". Skipping this job!\n'
                skip_reason = "Found a Bad Word in About Job"
                skip = True
                break
        if not skip and bool(get_runtime_value("security_clearance", False)) is False and ('polygraph' in job_description_low or 'clearance' in job_description_low or 'secret' in job_description_low):
            skip_message = f'\n{job_description}\n\nFound "Clearance" or "Polygraph". Skipping this job!\n'
            skip_reason = "Asking for Security clearance"
            skip = True
        if not skip:
            if bool(get_runtime_value("did_masters", False)) and 'master' in job_description_low:
...
def upload_resume(modal, resume: str) -> tuple[bool, str]:
    try:
        ...
        return True, os.path.basename(str(get_runtime_value("default_resume_path", resume)))
    except Exception:
