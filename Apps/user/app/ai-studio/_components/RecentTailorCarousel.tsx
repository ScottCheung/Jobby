...
export function documentTypeLabel(item: TailoredResume): string[] {
...
interface RecentTailorCarouselProps {
  ...
  onLoadAll?: () => Promise<TailoredResume[]>;
...
export function RecentTailorCarousel({
  ...
  return (
    <div
      ref={containerRef}
      className={cn(
        'w-full min-w-0 flex flex-col gap-2 transition-all',
        isSticky ?
          'bg-background-primary border-b border-border/40 shadow-xs py-2'
        : 'bg-transparent border-transparent shadow-none py-0',
        className,
      )}
    >
      {/* Header */}
      <div className='flex items-center gap-3 w-full min-w-0'>
        <span className='text-xs font-bold uppercase tracking-wider text-ink-secondary flex items-center gap-1.5 shrink-0'>
              ...
              return (
                <div
                  ...
                  className={cn(
                    ...
                    isSelected ?
                      'bg-popover dark:bg-zinc-900 border-2 border-primary shadow-md'
                    : 'bg-popover dark:bg-zinc-900 border border-primary/15 hover:border-primary/50 hover:shadow-xs hover:-translate-y-0.5',
                  )}
                >
