...
function isQuestionType(value: unknown): value is TestFieldType {
  return (
    typeof value === 'string' && QUESTION_TYPES.includes(value as TestFieldType)
  );
}
...
function fieldTypeFor(label: string): TestFieldType {
  ...
  if (
    /(upload|photo|transcript|work sample|cover letter|cv|resume)/.test(
      normalized,
    )
  )
    return 'file';
  if (/(email|e-mail)/.test(normalized)) return 'email';
  if (/(phone|telephone|contact number|mobile|cell)/.test(normalized))
    return 'tel';
  if (/(date|expiry|available from|graduation)/.test(normalized)) return 'date';
  if (
    /(salary|compensation|rate|gpa|percentage|years|age|number|code|extension|postcode|zip|hours|count|size|notice period)/.test(
      normalized,
    )
  )
    return 'number';
  if (
    /^(can|are|do|will|would|is|have you|do you now)|authorized|sponsorship|permanent residency|own a vehicle|work weekends|night shifts|overtime|right to work|driver'?s licence|white card|police check|working with children/.test(
      normalized,
    )
  )
    return 'radio';
      ...
      return {
        ...
        cases: group.questions
          .map((item, questionIndex) =>
            makeField(item, `${id}-${questionIndex}`, group.label, id),
          )
...
function activeFields(config: TestConfig): TestField[] {
  ...
  if (!Array.isArray(activeQuestions))
    return allCases.length ? allCases : [firstCase()];
  ...
  return activeQuestions
    .map((item, index) => {
      ...
      const group = configGroups.find(
        (candidate) =>
          candidate.label.toLowerCase() === groupLabel.toLowerCase(),
      );
...
function TestControl({
  ...
  const className =
    'w-full border border-primary bg-background px-3 text-sm text-ink-primary outline-none transition focus:border-primary focus:ring-1 focus:ring-primary/20';
  if (field.type === 'textarea') {
    return (
      <textarea
        ...
        className={`${className} min-h-20 py-2`}
      />
    );
  }
  ...
  if (field.type === 'radio') {
    return (
      <div
        className='flex min-h-10 flex-wrap items-center gap-x-4 gap-y-2'
        role='radiogroup'
        aria-label={field.label}
      >
        {field.options.map((option, index) => {
  ...
  if (field.type === 'checkbox') {
    return (
      <label className='inline-flex min-h-10 items-center gap-2 text-sm text-ink-primary'>
        <input
          id={controlId}
          name={controlId}
          type='checkbox'
  ...
  if (field.type === 'file') {
    return (
      <input
        ...
        aria-label={field.label}
        onChange={() => undefined}
        className={`${className} h-10 py-1.5`}
      />
    );
  }
  return (
...
function AddQuestion({
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    const result = onAdd({
      ...
      group:
        ...
        : groups.find((item) => item.id === group)?.label || 'Custom',
      type,
      required,
  ...
  return (
    <section className='border border-transparent bg-panel'>
      <div className='border-b border-primary/60 px-4 py-3'>
        <h2 className='text-sm font-semibold text-ink-primary'>
          Add your own question
        </h2>
        <p className='mt-0.5 text-xs text-ink-secondary'>
          Create a control that matches a real Australian application form.
        </p>
      </div>
      <form onSubmit={handleSubmit} className='grid gap-4 p-4'>
        <div className='grid gap-3 md:grid-cols-[minmax(0,1fr)_10rem_10rem]'>
          <label className='grid gap-1 text-xs font-medium text-ink-secondary'>
            ...
            <input
              ...
              placeholder='For example: Do you hold a current First Aid certificate?'
          ...
          <label className='grid gap-1 text-xs font-medium text-ink-secondary'>
            ...
            <select
              value={type}
              onChange={(event) =>
                handleTypeChange(event.target.value as TestFieldType)
              }
              className='h-10 border border-primary bg-background px-3 text-sm text-ink-primary outline-none focus:border-primary'
              disabled={disabled}
            >
        ...
        {group === 'custom' && (
          <label className='grid max-w-md gap-1 text-xs font-medium text-ink-secondary'>
            Group name
            <input
              value={customGroup}
              ...
              {options.map((option, index) => (
                ...
                >
                  <input
                    ...
                    className='h-9 border border-primary bg-background px-3 text-sm text-ink-primary outline-none focus:border-primary'
                    disabled={disabled}
                  />
                  <input
                    value={option.value}
                  ...
                  <button
                    ...
                    className='inline-flex size-9 items-center justify-center text-ink-secondary hover:text-red-600 disabled:opacity-40'
        ...
        <div className='flex flex-wrap items-center justify-between gap-3'>
          ...
          <button
            ...
            className='inline-flex h-10 items-center gap-2 bg-primary px-4 text-sm font-medium text-primary-foreground disabled:opacity-50'
          >
...
export default function AutofillTestPage() {
  const [fields, setFields] = useState<TestField[]>(() =>
    activeFields(testFieldsSeed as TestConfig),
  );
  const [availableGroups, setAvailableGroups] =
    useState<QuestionGroup[]>(seedGroups);
  ...
  useEffect(() => {
    ...
    void fetch('/api/test-autofill', { cache: 'no-store' })
      .then(async (response) => {
        if (!response.ok)
          throw new Error('Could not read the test question library.');
          ...
          if (localQuestions) {
            ...
            if (Array.isArray(parsed))
              nextFields = activeFields({
                ...config,
                activeQuestions: parsed as JsonQuestion[],
              });
          }
        } catch {
        ...
        if (!cancelled)
          setError(
            cause instanceof Error ?
              cause.message
            : 'Could not read the test question library.',
          );
      });
    return () => {
      cancelled = true;
  ...
  const activeIdentities = useMemo(
    () => new Set(fields.map(fieldIdentity)),
    [fields],
  );
  const filteredGroups = useMemo(() => {
    ...
    return availableGroups
      .map((group) => ({
        ...group,
        cases: group.cases.filter((field) =>
  ...
  const typeCounts = useMemo(
    () =>
      QUESTION_TYPES.map((type) => ({
        type,
        count: fields.filter((field) => field.type === type).length,
  ...
  const persistFields = async (next: TestField[]) => {
    ...
    try {
      window.localStorage.setItem(
        STORAGE_KEY,
        JSON.stringify(next.map(serializeField)),
      );
    } catch {
    ...
    try {
      ...
      if (version === saveVersion.current)
        setNotice('Saved for the next test run.');
    } catch {
  ...
  const addCases = (cases: TestField[]) => {
    const additions = cases.filter(
      (item) => !activeIdentities.has(fieldIdentity(item)),
    );
  ...
  const clearFields = () => {
    if (
      ...
      !window.confirm('Clear every question from this test form?')
    )
      return;
    setValues({});
    void persistFields([]);
  ...
  const runAutofill = async () => {
    ...
    setNotice('');
    try {
      const response: FormAutofillTestResponse = await api.testFormAutofill({
        platform: 'generic',
        fields,
      });
      ...
      setError(
        cause instanceof Error ? cause.message : 'Could not run Autofill.',
      );
    } finally {
      setRunning(false);
    }
  ...
  return (
    <main className='mx-auto flex w-full max-w-6xl flex-col gap-5 pb-10'>
      <header className='flex flex-col gap-4 border-b border-primary/60 pb-4 sm:flex-row sm:items-end sm:justify-between'>
        <div>
          <h1 className='title-page text-ink-primary'>Autofill Test Form</h1>
          <p className='mt-1 text-sm text-ink-secondary'>
            Build a realistic Australian job application form and check which
            saved answers Autofill can use.
          </p>
        </div>
        <div className='flex shrink-0 items-center gap-2'>
          <button
            type='button'
            onClick={clearFields}
            disabled={!fields.length || saving}
            className='inline-flex h-10 items-center gap-2 border border-primary px-3 text-sm text-ink-secondary hover:border-red-300 hover:text-red-600 disabled:opacity-50'
          ...
          <button
            ...
            disabled={running || !fields.length || saving}
            className='inline-flex h-10 items-center gap-2 bg-primary px-4 text-sm font-medium text-primary-foreground disabled:opacity-50'
          >
      ...
      <section className='flex flex-wrap items-center gap-x-4 gap-y-2 border-y border-primary/60 py-3'>
        <span className='text-sm font-semibold text-ink-primary'>
          {fields.length} questions
        </span>
        {typeCounts.map(({ type, count }) => (
          <span key={type} className='text-xs text-ink-secondary'>
            {count} {typeLabel(type).toLowerCase()}
          </span>
        ))}
        {saving && (
          <span className='text-xs text-ink-secondary'>Saving...</span>
        )}
      </section>

      <section className='border border-transparent bg-panel'>
        <div className='flex flex-col gap-3 border-b border-primary/60 px-4 py-3 sm:flex-row sm:items-center sm:justify-between'>
          <div>
            <h2 className='text-sm font-semibold text-ink-primary'>
              Question library
            </h2>
            <p className='mt-0.5 text-xs text-ink-secondary'>
              Add a whole category or pick individual sample questions.
            </p>
          </div>
          <div className='flex items-center gap-2'>
            <input
              ...
              aria-label='Search sample questions'
              className='h-9 w-full border border-primary bg-background px-3 text-sm text-ink-primary outline-none focus:border-primary sm:w-52'
            />
            <button
              type='button'
              onClick={() =>
                addCases(availableGroups.flatMap((group) => group.cases))
              }
              disabled={
              ...
              className='inline-flex h-9 shrink-0 items-center gap-1.5 border border-primary/40 bg-primary/5 px-3 text-xs font-medium text-primary hover:bg-primary/10 disabled:opacity-50'
            >
              <Plus className='size-3.5' />
              Add all
            </button>
        ...
        <div className='divide-y divide-border/50'>
          {filteredGroups.map((group) => {
            const remaining = group.cases.filter(
              (item) => !activeIdentities.has(fieldIdentity(item)),
            );
            return (
              <div
              ...
              >
                <div className='min-w-0'>
                  ...
                  <p className='mt-0.5 text-xs text-ink-secondary'>
                    {group.description ||
                      `${group.cases.length} sample questions`}
                  </p>
                ...
                <button
                  type='button'
                  onClick={() => addCases(remaining)}
                  disabled={!remaining.length || saving}
                  className='inline-flex h-8 shrink-0 items-center justify-center gap-1.5 border border-primary px-3 text-xs text-ink-primary hover:bg-background-secondary disabled:opacity-50'
                >
                  <Plus className='size-3.5' />
                  {remaining.length ? `Add ${remaining.length}` : 'Added'}
                </button>
              </div>
            );
          })}
          {!filteredGroups.length && (
            <p className='px-4 py-8 text-center text-sm text-ink-secondary'>
              No sample questions match that search.
            </p>
          )}
        </div>
      </section>
      ...
      >
        <section className='border border-transparent bg-panel'>
          <div className='border-b border-primary/60 px-4 py-3'>
            ...
            <p className='mt-0.5 text-xs text-ink-secondary'>
              Edit the controls below, then run Autofill to see the match and
          ...
          <div className='divide-y divide-border/50'>
            {fields.map((field) => {
              ...
              return (
                <div
                  key={field.key}
                  className='grid gap-3 px-4 py-4 lg:grid-cols-[minmax(0,1fr)_minmax(18rem,1fr)_minmax(0,1fr)_auto] lg:items-start'
                >
                  <div className='min-w-0'>
                    <div className='flex flex-wrap items-center gap-2'>
                      ...
                      <span className='border border-transparent px-1.5 py-0.5 text-[10px] text-ink-secondary'>
                    ...
                    : <label
                        htmlFor={controlId}
                        className='mt-1 block text-sm font-medium text-ink-primary'
                      >
                        {field.label}
                        {field.required && (
                          <span className='ml-1 text-red-600'>*</span>
                  ...
                  <div className='min-w-0 text-xs text-ink-secondary'>
                    ...
                    {trace?.intent_key && (
                      <p className='mt-1 break-words'>
                        Intent:{' '}
                        <code className='text-primary'>{trace.intent_key}</code>
                      </p>
                    )}
                    {trace?.source && (
                      <p className='mt-1'>
                    ...
                    {trace?.reason && (
                      <p className='mt-1 text-amber-700'>{trace.reason}</p>
                    )}
                  </div>
                  <button
                    type='button'
                    ...
                    disabled={saving}
                    className='inline-flex size-8 items-center justify-center text-ink-secondary hover:text-red-600 disabled:opacity-50'
                  >
                    <Trash2 className='size-4' />
                  </button>
                </div>
            ...
            {!fields.length && (
              <p className='px-4 py-10 text-center text-sm text-ink-secondary'>
                Add a question from the library or create your own below.
              </p>
            )}
          </div>
        </section>
