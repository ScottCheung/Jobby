...
export function CustomizePlanModal({
  ...
  const handleFormSubmit = () => {
    if (
      ...
      alert('Please select at least one question for the plan.');
      return;
    }
        ...
        <div className='flex-1 p-6 grid grid-cols-1 md:grid-cols-3 overflow-hidden min-h-0 divide-y md:divide-y-0 md:divide-x divide-dashed divide-primary/20 gap-6'>
          ...
          <div className='md:col-span-2 overflow-y-auto  flex flex-col pr-6 custom-scrollbar-primary'>
            ...
            <div className='flex flex-col gap-2'>
              ...
              />
              <div className='flex justify-between text-xs text-ink-secondary/70 px-1 font-medium'>
                <span>3 Days</span>
                <span>14 Days</span>
                <span>30 Days</span>
                <span>60 Days</span>
            ...
            <div className='flex items-center justify-between py-2 border-t border-b border-zinc-100 dark:border-zinc-800/60'>
              <div className='flex flex-col gap-1 pr-4'>
                ...
                <span className='text-xs text-ink-secondary leading-relaxed max-w-lg'>
              ...
              <div className='flex flex-col gap-3 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-4 bg-zinc-50/20 dark:bg-zinc-900/10 min-h-[420px] transition-all duration-300'>
                <div className='flex items-center gap-2 shrink-0'>
                  ...
                  <button
                    ...
                    className={cn(
                      'flex items-center gap-2 px-3 py-2.5 text-sm font-medium rounded-xl border transition-all shrink-0 h-[46px]',
                      showManualFilters ||
                        filterCategoryIds.length > 0 ||
                        filterImportances.length > 0 ||
                        filterFrequencies.length > 0 ?
                ...
                <div className='flex-1 overflow-y-auto flex flex-col gap-2 pr-1 custom-scrollbar-primary'>
                  {filteredQuestions.map((q) => {
                    const isSelected = manuallySelectedQuestionIds.includes(
                      q.id,
                    );
                    return (
                      <div
                        ...
                        className={cn(
                          'flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition-all hover:bg-zinc-100/50 dark:hover:bg-zinc-850/50',
                          isSelected ?
                            'border-primary bg-primary/5'
                          : 'border-zinc-200/50 dark:border-zinc-850/50 bg-panel',
                        )}
                      >
                        <Checkbox
                          checked={isSelected}
                  ...
                  {filteredQuestions.length === 0 && (
                    <div className='flex-1 flex items-center justify-center text-sm text-ink-secondary italic p-8'>
                      No questions found.
                    </div>
                  )}
                </div>
          ...
          <div className=''>
            <div className='flex flex-col gap-5'>
              <h4 className='text-sm font-semibold text-ink-secondary uppercase tracking-wider'>
                Summary
              </h4>
              ...
              <div className='status-panel px-6! flex flex-col gap-4 relative overflow-hidden'>
                ...
                <div className='grid grid-cols-2 gap-4 pt-2 text-sm font-medium'>
                  <div className='flex flex-col gap-1'>
                    <span className='text-ink-primary font-bold text-base'>
                      {duration} Days
                    </span>
                    <span className='text-xs text-ink-secondary/50 flex items-center gap-1'>
                      <Clock className='w-3.5 h-3.5 text-zinc-400' /> Duration
                    </span>
                  </div>
                  <div className='flex flex-col gap-1'>
                    <span className='text-ink-primary font-bold text-base'>
                      {selectedQsCount} Questions
                    </span>
                    <span className='text-xs text-ink-secondary/50 flex items-center gap-1'>
