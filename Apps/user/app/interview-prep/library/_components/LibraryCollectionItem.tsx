...
export function LibraryCollectionItem({
  ...
  return (
    <div className='group/item rounded-xl border border-transparent transition-colors hover:border-primary/60 hover:bg-background-secondary/25 focus-within:border-primary/60 focus-within:bg-background-secondary/25'>
      <button
        type='button'
        onClick={onSelect}
        className={cn(
      ...
      >
        ...
        <span
          className={cn(
            ...
            : 'bg-zinc-200 text-ink-secondary dark:bg-zinc-700/80',
          )}
        >
          {count}
        </span>
          ...
          <div className='border-t border-primary/40 pt-2'>
            <div className='flex items-center justify-between gap-2 text-[10px]'>
              <span className='text-ink-secondary'>
                {isEmpty ?
                  'No questions yet'
                : `${collection.user_active_question_count}/${collection.question_count} in Library`
                }
              </span>
              <span
                className={cn(
              ...
              {isOwned && (
                <>
                  <button
                    ...
                    disabled={isBusy}
                    className='inline-flex items-center gap-1 rounded-lg border border-primary px-2 py-1 text-[10px] font-semibold text-ink-primary transition-colors hover:bg-background-secondary disabled:opacity-60'
                  >
