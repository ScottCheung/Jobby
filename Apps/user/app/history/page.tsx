  ...
  return (
    <div className='max-w-5xl mx-auto pb-24'>
      <div className='flex items-center justify-between mb-8'>
        <div>
          <h1 className='title-page text-ink-primary mb-2'>
            History & Rewards
          </h1>
          <p className='text-ink-secondary'>
            View your past practice sessions and track your XP earnings.
          </p>
        </div>
      </div>

      <div className='flex gap-4 border-b border-primary mb-6'>
        <button
      ...
      : activeTab === 'practice' ?
        <div className='space-y-4'>
          ...
          : practiceRecords.map((record) => (
              <div
                key={record.id}
                className='bg-panel border border-primary rounded-xl p-5 hover:border-primary/20 transition-colors'
              >
                <div className='flex flex-col md:flex-row md:items-center justify-between gap-4'>
                  <div className='flex-1'>
                    ...
                    <h3 className='title-card mb-1'>
                      {questions[record.question_id]?.title ||
                        'Unknown Question'}
                    </h3>
                  ...
                  {record.audio_records && record.audio_records.length > 0 && (
                    <div className='shrink-0 flex items-center'>
                      <audio
                        controls
                        className='h-10 max-w-[200px] md:max-w-[250px]'
                      >
                        <source
                          src={record.audio_records[0].url_path}
                          type='audio/webm'
                        />
                      </audio>
                    </div>
      ...
      : <div className='flex flex-col lg:flex-row gap-6 items-start'>
          <div className='flex-1 w-full space-y-4'>
            {transactions.length === 0 ?
              <EmptyPlaceHolder
                ...
                className='border-dashed py-12'
              />
            : <div className='bg-panel border border-primary rounded-xl overflow-hidden'>
                <table className='w-full text-left'>
                  <thead>
                    <tr className='label-overline bg-background-secondary/50 border-b border-primary text-ink-secondary'>
                  ...
                  <tbody className='divide-y divide-border'>
                    {transactions.map((tx) => (
                      ...
                      >
                        ...
                        <td className='label px-5 py-4'>{tx.reason}</td>
          ...
          <div className='w-full lg:w-72 shrink-0 space-y-4'>
            <div className='bg-panel border border-primary rounded-xl p-5'>
              ...
              <ul className='body-md space-y-4 text-ink-secondary'>
                <li className='flex items-start gap-2'>
                  ...
                  <span>
                    ...
                    when you practice a question for the first time each day.
                  </span>
                </li>
                <li className='flex items-start gap-2'>
                  <PlayCircle className='w-4 h-4 text-emerald-500 mt-0.5 shrink-0' />
                  <span>
                    Practicing the same question multiple times in one day will{' '}
                    <strong className='text-ink-primary'>not</strong> yield
                    duplicate rewards.
                  </span>
                </li>
                <li className='flex items-start gap-2'>
                  ...
                  <span>
                    <strong className='text-ink-primary'>+500 XP Bonus</strong>{' '}
                    for every 7 consecutive days you practice (7-Day Streak).
                  </span>
                </li>
                <li className='flex items-start gap-2'>
                  <Gem className='w-4 h-4 text-purple-500 mt-0.5 shrink-0' />
                  <span>
                    <strong className='text-ink-primary'>Leveling Up:</strong>{' '}
                    Higher levels require progressively more XP. Keep practicing
