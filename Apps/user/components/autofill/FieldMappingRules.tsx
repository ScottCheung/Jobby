...
function MappingRuleEditor({
  ...
  const [coreKey, setCoreKey] = useState(
    value?.core_field_key || coreFields[0]?.core_field_key || '',
  );
  const [scene, setScene] = useState(value?.scene || 'generic');
  const [features, setFeatures] = useState(
    (value?.semantic_features || []).join(', '),
  );
    ...
    <div className='flex w-full flex-col'>
      <header className='header'>
        <h2 className='title-section text-ink-primary'>
          {value ? 'Edit mapping rule' : 'New mapping rule'}
        </h2>
        <Button
          variant='toolbar'
          size='icon'
      ...
      <div className='body grid grid-cols-1 gap-4 py-5! sm:grid-cols-2'>
        ...
        <div>
          ...
          <select
            ...
            className='h-11 w-full rounded-md border border-primary bg-background px-3 text-sm text-ink-primary'
          >
            {!coreFields.length && (
              <option value=''>No core fields available</option>
            )}
            {coreFieldCategories.map((category) => {
              const fields = coreFields.filter(
                (field) =>
                  coreFieldCategoryForKey(field.core_field_key) === category.id,
              );
        ...
        <Field
          ...
          value={scene}
          onChange={setScene}
          hint='generic, job_application, visa_application'
        />
        <Field
          label='Semantic features'
          value={features}
          onChange={setFeatures}
          hint='Comma-separated intent signals'
        />
      </div>
      <footer className='footer'>
        ...
        <Button
          ...
          isLoading={saving}
          disabled={!alias.trim() || !coreKey.trim()}
        >
          Save rule
        </Button>
      </footer>
...
export function FieldMappingRulesPanel({
  id = 'field-mappings',
}: {
  id?: string;
}) {
  ...
  const editMappingRule = (rule?: FieldMappingRule) =>
    openModal({
      ...
      content: (
        <MappingRuleEditor
          ...
          onSave={async (payload) => {
            const saved =
              rule ?
                await api.updateFieldMappingRule({ ...rule, ...payload })
              : await api.createFieldMappingRule(payload);
            setMappingRules((current) =>
              rule ?
    ...
    <section id={id} className='scroll-mt-5'>
      <CardWithNorth title='Field Mapping Rules' size='sm'>
        <div className='mb-4 flex items-center gap-2 text-xs text-ink-secondary'>
          ...
          <span>
            {customRules.length} custom · {systemCount} system
          </span>
          <Button
            variant='secondary'
            size='sm'
        ...
        : <div className='divide-y divide-border/60'>
              ...
              <div key={rule.id} className='flex items-center gap-3 py-3'>
                <div className='min-w-0 flex-1'>
                  ...
                  <p className='truncate text-xs text-ink-secondary'>
                    {rule.scene} -&gt; {rule.core_field_key} | {rule.confidence}
                    %
                  </p>
                </div>
                <Button
                ...
                <Button
                  ...
                  size='icon'
                  Icon={Trash2}
                  onClick={() => void deleteMappingRule(rule)}
            ...
            {customRules.length === 0 && (
              <p className='py-6 text-center text-sm text-ink-secondary'>
                Manual corrections confirmed in the extension will appear here.
              </p>
            )}
          </div>
        }
        {!loading && !coreFields.length && (
          <p className='mt-3 text-xs text-ink-secondary'>
            Add a core Profile value before creating a custom mapping.
          </p>
        )}
      </CardWithNorth>
    </section>
