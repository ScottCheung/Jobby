...
export function parseDescriptionBlocks(rawText: string | null | undefined): ContentBlock[] {
  if (!rawText) return [];
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const isExplicitHeader = HEADER_PATTERN.test(line) || /^[A-Z\u4e00-\u9fa5][A-Za-z0-9\u4e00-\u9fa5\s/&'-]{1,60}:$/.test(line) || line.endsWith(':');
    const isHeaderPrecedingBullets = !isBullet && nextNonEmptyIsBullet && line.length <= 90 && !/[.!?!。！？]$/.test(line);
