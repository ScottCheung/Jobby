...
(function () {
    ...
    switch (issue.code) {
      ...
      case ZodIssueCode.too_small:
        if (issue.type === 'array')
          ...
          message = `Number must be ${
            issue.exact ? `exactly equal to `
            : issue.inclusive ? `greater than or equal to `
            : `greater than `
          }${issue.minimum}`;
      ...
      case ZodIssueCode.too_big:
        if (issue.type === 'array')
          ...
          message = `Number must be ${
            issue.exact ? `exactly`
            : issue.inclusive ? `less than or equal to`
            : `less than`
          } ${issue.maximum}`;
        else if (issue.type === 'bigint')
          ...
          message = `Date must be ${
            issue.exact ? `exactly`
            : issue.inclusive ? `smaller than or equal to`
            : `smaller than`
  ...
  class ZodType {
    ...
    async safeParseAsync(data, params) {
      ...
      const result2 = await (isAsync(maybeAsyncResult) ? maybeAsyncResult : (
  ...
  class ZodString extends ZodType {
    ...
    datetime(options) {
      ...
      return this._addCheck({
        ...
        precision:
          (
            typeof (options == null ? void 0 : options.precision) ===
            'undefined'
          ) ?
            null
          : options == null ? void 0
    ...
    _parse(input) {
      ...
      function handleResults(results) {
        ...
        const unionErrors = results.map(
          (result2) => new ZodError(result2.ctx.common.issues),
        );
        addIssueToContext(ctx, {
  ...
  class ZodSet extends ZodType {
    _parse(input) {
      ...
      const elements2 = [...ctx.data.values()].map((item, i) =>
        valueType._parse(new ParseInputLazyPath(ctx, item, ctx.path, i)),
      );
      if (ctx.common.async) {
        return Promise.all(elements2).then((elements3) =>
          finalizeSet(elements3),
        );
      } else {
  ...
  class ZodEffects extends ZodType {
    ...
    sourceType() {
      return (
          this._def.schema._def.typeName === ZodFirstPartyTypeKind.ZodEffects
        ) ?
  ...
  class ZodReadonly extends ZodType {
    _parse(input) {
      ...
      return isAsync(result2) ?
          result2.then((data) => freeze(data))
        : freeze(result2);
    }
    unwrap() {
      return this._def.innerType;
  ...
  const jobSnapshotFields = {
    ...
    datePosted: stringType().optional(),
  ...
  const genericJobSnapshotSchema = objectType({
    platform: literalType('generic'),
    ...jobSnapshotFields,
  });
  const indeedJobSnapshotSchema = objectType({
    platform: literalType('indeed'),
    ...jobSnapshotFields,
  });
  ...
  const pageInspectionSchema = discriminatedUnionType('kind', [
    ...
    objectType({
      ...
      platform: enumType(['seek', 'linkedin', 'indeed']),
      url: stringType().url(),
  ...
  const formFieldObservationSchema = objectType({
    ...
    frameId: numberType().int().nonnegative().optional(),
  ...
  const fileUploadInstructionSchema = objectType({
    type: literalType('content.upload-file'),
    commandId: stringType().min(1).max(128),
    ...
    contentBase64: stringType().min(1).max(14e6),
  });
  objectType({
  ...
  const linkedinApplicationActionSchema = enumType([
    'previous',
    'next',
    'submit',
  ]);
  ...
  const TECHNOLOGY_RULES = [
    {
      label: 'ASP.NET Core',
      pattern: /(?:^|[^A-Za-z0-9])ASP\.NET\s+Core(?:$|[^A-Za-z0-9])/i,
    },
    {
      label: 'Azure DevOps',
      pattern: /(?:^|[^A-Za-z0-9])Azure\s+DevOps(?:$|[^A-Za-z0-9])/i,
    },
    {
      label: 'GitHub Actions',
      pattern: /(?:^|[^A-Za-z0-9])GitHub\s+Actions(?:$|[^A-Za-z0-9])/i,
    },
    {
      label: 'GitLab CI/CD',
      pattern: /(?:^|[^A-Za-z0-9])GitLab\s+CI\s*\/\s*CD(?:$|[^A-Za-z0-9])/i,
    },
    {
      label: 'Microsoft SQL Server',
      pattern: /(?:^|[^A-Za-z0-9])Microsoft\s+SQL\s+Server(?:$|[^A-Za-z0-9])/i,
    },
    {
      label: 'Google Cloud',
      pattern: /(?:^|[^A-Za-z0-9])Google\s+Cloud(?:$|[^A-Za-z0-9])/i,
    },
    {
      label: 'Spring Boot',
      pattern: /(?:^|[^A-Za-z0-9])Spring\s+Boot(?:$|[^A-Za-z0-9])/i,
    },
    {
      label: 'React Native',
      pattern: /(?:^|[^A-Za-z0-9])React\s+Native(?:$|[^A-Za-z0-9])/i,
    },
    {
      label: 'Node.js',
      pattern: /(?:^|[^A-Za-z0-9])Node\.js(?:$|[^A-Za-z0-9])/i,
    },
    {
      label: 'Next.js',
      pattern: /(?:^|[^A-Za-z0-9])Next\.js(?:$|[^A-Za-z0-9])/i,
    },
    {
      label: 'Express.js',
      pattern: /(?:^|[^A-Za-z0-9])Express(?:\.js)?(?:$|[^A-Za-z0-9])/i,
    },
    {
      label: 'REST APIs',
      pattern: /(?:^|[^A-Za-z0-9])REST(?:ful)?\s+APIs?(?:$|[^A-Za-z0-9])/i,
    },
    {
      label: 'CI/CD',
      pattern: /(?:^|[^A-Za-z0-9])CI\s*\/\s*CD(?:$|[^A-Za-z0-9])/i,
    },
  ...
  function cleanText$9(value) {
    return (value || '').replace(/\s+/g, ' ').trim();
  }
  function extractTechnologyKeywords(value) {
    const text = cleanText$9(value);
    if (!text) return [];
  ...
  function cleanText$8(value) {
    return (value || '').replace(/\s+/g, ' ').trim();
  }
  function firstText$2(selectors) {
    for (const selector of selectors) {
      const element = document.querySelector(selector);
      const text = cleanText$8(element == null ? void 0 : element.textContent);
      if (text) return text;
    }
    return '';
  }
  ...
  function hasApplyAction$1() {
    return SEEK_SELECTORS.apply.some((selector) =>
  ...
  function readSeekPage() {
    ...
    if (!jobId) {
      return {
        ...
        platform: 'seek',
        url,
        reason: 'The URL does not identify a SEEK job.',
      };
    }
    const title = firstText$2(SEEK_SELECTORS.title);
    if (!title) {
      return {
        kind: 'not_job_page',
        platform: 'seek',
        url,
        reason: 'The job title is not available yet.',
      };
    }
    const description = firstText$2(SEEK_SELECTORS.description);
    const snapshot = {
      platform: 'seek',
      externalId: jobId,
      url,
      title,
      company: firstText$2(SEEK_SELECTORS.company) || 'Unknown company',
      location: firstText$2(SEEK_SELECTORS.location) || void 0,
      description: description || void 0,
      technologies: extractTechnologyKeywords(description),
      easyApply: hasApplyAction$1(),
    };
  ...
  function cleanLabel(value) {
    return cleanText$7(value)
      .replace(/\s*(?:Required|必填|\*)\s*$/gi, '')
      .trim();
  }
  ...
  function optionLabelFor$1(element, scope) {
    var _a2;
    const ariaLabel = cleanText$7(element.getAttribute('aria-label'));
    if (ariaLabel) return cleanLabel(ariaLabel);
    const id = cleanText$7(element.id);
    if (id) {
      const label = scope.querySelector(`label[for='${CSS.escape(id)}']`);
      const text = cleanText$7(label == null ? void 0 : label.textContent);
      if (text) return cleanLabel(text);
    }
    const parentLabel = cleanText$7(
      (_a2 = element.closest('label')) == null ? void 0 : _a2.textContent,
    );
    if (parentTextIsDistinct(parentLabel, element))
      return cleanLabel(parentLabel);
    if (element instanceof HTMLInputElement && element.value)
      return cleanText$7(element.value);
    return 'Option';
  }
  function parentTextIsDistinct(text, element) {
    ...
    const legend = cleanText$7(
      (
        (_a2 = fieldset == null ? void 0 : fieldset.querySelector('legend')) ==
          null
      ) ?
        void 0
      : _a2.textContent,
  ...
  function labelFor$2(element, scope) {
    ...
    if (
      ...
      const legend2 = cleanText$7(
        (
          (_a2 =
            fieldset2 == null ? void 0 : fieldset2.querySelector('legend')) ==
            null
        ) ?
          void 0
        : _a2.textContent,
    ...
    const labelledBy = cleanText$7(element.getAttribute('aria-label'));
    ...
    const parentText = cleanText$7(
      parentLabel == null ? void 0 : parentLabel.textContent,
    );
    if (parentText) return cleanLabel(parentText);
    const fieldset = element.closest('fieldset');
    const legend = cleanText$7(
      (
        (_b2 = fieldset == null ? void 0 : fieldset.querySelector('legend')) ==
          null
      ) ?
        void 0
      : _b2.textContent,
    ...
    const placeholder = cleanText$7(element.getAttribute('placeholder'));
  ...
  function requiredFor(element) {
    ...
    const fieldset = element.closest('fieldset');
    return Boolean(
      (fieldset == null ? void 0 : fieldset.hasAttribute('required')) ||
      (fieldset == null ? void 0 : fieldset.getAttribute('aria-required')) ===
        'true',
    );
  }
  function optionsFor(element, scope) {
    if (element instanceof HTMLSelectElement) {
      return Array.from(element.options).map((option) => ({
        label: cleanText$7(option.textContent) || option.value,
        value: option.value,
      }));
    }
    if (
  ...
  function currentValue(element, type, scope) {
    ...
    if (
      ...
      return (
        optionLabelFor$1(checkedRadio, scope) || checkedRadio.value || 'true'
      );
    }
    ...
    if (element instanceof HTMLSelectElement) {
      return Array.from(element.selectedOptions)
        .map((option) => cleanText$7(option.textContent))
        .join(', ');
    }
    return cleanText$7(element.value);
  }
  function isFilled(element, type, scope) {
    ...
    if (type === 'file')
      return Boolean(
        element instanceof HTMLInputElement &&
        ((_a2 = element.files) == null ? void 0 : _a2.length),
      );
    if (
      element instanceof HTMLInputElement &&
      type === 'radio' &&
  ...
  function fileUploadLabelFor(element, scope) {
    ...
    const controller =
      element.id ?
        root.querySelector(`[aria-controls='${CSS.escape(element.id)}']`)
      : null;
    const text = cleanText$7(
      (explicitLabel == null ? void 0 : explicitLabel.textContent) ||
        (controller == null ? void 0 : controller.getAttribute('aria-label')) ||
        (controller == null ? void 0 : controller.textContent) ||
        element.getAttribute('aria-label') ||
        element.getAttribute('name'),
    );
    if (/upload\s+resume|resume\s+upload/i.test(text)) return 'Resume';
    if (/upload\s+(?:cv|cover\s+letter)/i.test(text))
      return cleanLabel(text.replace(/^upload\s+/i, ''));
  ...
  function selectedDocumentFor(element, scope) {
    ...
    const selectedLabel = Array.from(
      ...
      /^deselect\s+(?:resume|cv|cover\s+letter)\s+/i.test(
        cleanText$7(label.textContent),
      ),
    );
    if (!selectedLabel) return void 0;
    const name = cleanText$7(selectedLabel.textContent)
      .replace(/^deselect\s+(?:resume|cv|cover\s+letter)\s+/i, '')
      .trim();
    if (!name) return void 0;
    const cardText = cleanText$7(
      (
        (_a2 = selectedLabel.closest('.jobs-document-upload-redesign-card')) ==
          null
      ) ?
        void 0
      : _a2.textContent,
  ...
  function documentOptionsFor(element, scope) {
    ...
    return Array.from(
      ...
      .map((label) => {
        const text = cleanText$7(label.textContent);
        const match = text.match(
          /^(?:deselect|select)\s+(?:resume|cv|cover\s+letter)\s+(.+)$/i,
        );
        const value = cleanText$7(label.htmlFor);
        return (match == null ? void 0 : match[1]) && value ?
            { label: match[1].trim(), value }
          : null;
      })
  ...
  function fileRequiredFor(element, scope) {
    ...
    const explicitLabel =
      element.id ?
        root.querySelector(`label[for='${CSS.escape(element.id)}']`)
      : null;
    const nearbyText = cleanText$7(
      (
        (_a2 =
          explicitLabel == null ? void 0 : (
            explicitLabel.closest('fieldset, section, div')
          )) == null
      ) ?
        void 0
      : _a2.textContent,
    ...
    return (
      /\*\s*$/.test(
        cleanText$7(explicitLabel == null ? void 0 : explicitLabel.textContent),
  ...
  function isPresentedFileInput(element, scope) {
    ...
    return Array.from(
      uploader.querySelectorAll("button, [role='button'], label"),
  ...
  function visibleControlsInScope(scope) {
    return controlsInScope(scope).filter(
      (element) => isVisibleElement(element) && isInspectableControl(element),
    );
  }
  function fieldKeyFor(element, index) {
    return (
      cleanText$7(element.id) ||
      cleanText$7(element.getAttribute('name')) ||
      `field-${index + 1}`
    );
  }
  function inspectVisibleFormFields(scope = document) {
    ...
    for (
      let index = 0;
      index < visibleControls.length && result2.length < 200;
      index += 1
    ) {
      ...
      if (
        element instanceof HTMLInputElement &&
        isDocumentSelectionRadio(element)
      )
        continue;
      if (
        type === 'radio' &&
      ...
      result2.push({
        ...
        id: cleanText$7(element.id) || void 0,
        name: cleanText$7(element.getAttribute('name')) || void 0,
        type,
    ...
    for (
      let index = 0;
      index < fileInputs.length && result2.length < 200;
      index += 1
    ) {
      ...
      result2.push({
        key,
        id: cleanText$7(input.id) || void 0,
        name: cleanText$7(input.getAttribute('name')) || void 0,
        type: 'file',
        label: fileUploadLabelFor(input, scope),
        required: fileRequiredFor(input, scope),
        filled: Boolean(
          (selectedFile && selectedFile.size > 0) ||
          (selectedDocument == null ? void 0 : selectedDocument.accepted),
        ),
        sensitive: true,
  ...
  const ACTION_SELECTOR$1 =
    "button, input[type='button'], input[type='submit'], [role='button']";
  function cleanText$6(value) {
    return (value || '').replace(/\s+/g, ' ').trim();
  }
  function isVisible$4(element) {
    const style = window.getComputedStyle(element);
    const rect = element.getBoundingClientRect();
    return (
      style.display !== 'none' &&
  ...
  function matchesAction(label, action) {
    if (action === 'previous')
      return /(?:back|previous|返回|上一步)/i.test(label);
    if (action === 'submit')
      return /(?:submit|apply|send|finish|提交|申请|发送|完成)/i.test(label);
    return (
  ...
  function getSeekApplicationAction(action) {
    return (
      Array.from(document.querySelectorAll(ACTION_SELECTOR$1)).find(
        (element) =>
          isVisible$4(element) &&
          isEnabled$1(element) &&
          matchesAction(labelFor$1(element), action),
      ) || null
    );
  }
  function getSeekApplicationActionLabel() {
    const action =
      getSeekApplicationAction('submit') ||
      getSeekApplicationAction('next') ||
      getSeekApplicationAction('previous');
  ...
  async function clickSeekApplicationAction(action) {
    ...
    if (!button) {
      return {
        status: 'unavailable',
        message:
          action === 'previous' ? 'The SEEK Back action is not available.'
          : action === 'next' ? 'The SEEK Continue action is not available.'
    ...
    return {
      status: 'clicked',
      message:
        action === 'previous' ? 'SEEK application moved to the previous step.'
        : action === 'next' ? 'SEEK application continued to the next step.'
  ...
  function isApplicationPage(url) {
    ...
    const bodyText = cleanText$5(
      (_a2 = document.body) == null ? void 0 : _a2.textContent,
    );
    return (
  ...
  const APPLICATION_ROOT_SELECTOR = [
    ...
    (selector, index, selectors) => selectors.indexOf(selector) === index,
  );
  ...
  function extractCleanElementText(element) {
    ...
    const rawText = cleanText$4(clone.textContent);
    return rawText
      .replace(
  ...
  function firstText$1(root, selectors) {
    for (const selector of selectors) {
  ...
  function descriptionText(element) {
    ...
    return cleanText$4(clone.innerText || clone.textContent);
  }
  ...
  function descriptionFromHeading(root) {
    const heading = deepElements(root).find((element) => {
      const text2 = cleanText$4(element.textContent);
      if (text2.length > 80) return false;
      return /^(?:about\s+the\s+job|job\s+description|job\s+details|职位描述|工作描述)$/i.test(
        text2,
      );
    });
    if (!heading) return '';
    ...
    const headingText = cleanText$4(heading.textContent);
    const body = cleanText$4(text.replace(headingText, ''));
    return body.length >= 40 ? body : '';
  }
  function findVisible(root, selectors, predicate = () => true) {
    const elements2 = deepElements(root);
    for (const selector of selectors) {
      const element = elements2.find(
        (candidate) =>
          candidate.matches(selector) &&
          predicate(candidate) &&
          isVisible$3(candidate) &&
          isEnabled(candidate),
      );
  ...
  function normalized$1(value) {
    return cleanText$4(value).toLowerCase();
  }
  ...
  const JOB_ROLE_KEYWORDS =
    /\b(?:engineer|developer|architect|lead|principal|senior|junior|mid|staff|manager|director|consultant|analyst|specialist|designer|administrator|coordinator|officer|executive|head|vp|intern|graduate|associate|agent|advisor|operator|technician|contractor)\b/i;
  function isLikelyTitle(value, company) {
    const text = cleanText$4(value);
    ...
    if (/[·•]/.test(text) || /\b(?:ago|applicants?)\b/i.test(text))
      return false;
    if (company) {
      ...
      if (
        !JOB_ROLE_KEYWORDS.test(text) &&
        normComp.length > 3 &&
        (normComp.includes(normText) || normText.includes(normComp))
      ) {
        return false;
      }
    }
  ...
  function isPureLocation(value) {
    ...
    return /^[A-Za-z\s.-]+,\s*(?:NSW|VIC|QLD|WA|SA|TAS|ACT|NT|Australia|New South Wales|Victoria|Queensland|Western Australia|South Australia|Tasmania|Australian Capital Territory|Northern Territory)(?:,\s*Australia)?$/i.test(
      text,
    );
  }
  function titleFromMain(company) {
    ...
    const paragraphs = Array.from(root.querySelectorAll('p'))
      .map((element) => cleanText$4(element.textContent))
      .filter(Boolean);
    const companyIndex = paragraphs.findIndex((text) => text === companyName);
    if (companyIndex < 0) return '';
    return (
      paragraphs
        .slice(companyIndex + 1, companyIndex + 5)
        .find((t) => isLikelyTitle(t, companyName)) || ''
    );
  }
  function titleFromDocument(jobId, company) {
    var _a2;
    if (!jobId) return '';
    const rawTitle = cleanText$4(document.title);
    const parts = rawTitle.split(/\s*\|\s*/);
    for (const part of parts) {
      const cleanedPart = part
        .replace(/\s*-\s*LinkedIn$/i, '')
        .replace(/\b(?:LinkedIn|Search|Jobs?)\b/gi, '')
        .trim();
      const candidate =
        cleanedPart.split(/\s+hiring\s+/i)[1] ||
        cleanedPart.split(/\s+is\s+hiring\s+/i)[1] ||
        cleanedPart;
      const titleOnly =
        ((_a2 = candidate.split(/\s+in\s+[^|-]+$/i)[0]) == null ?
          void 0
        : _a2.trim()) || candidate;
  ...
  function titleFromJobLink(jobId, company) {
    ...
    for (const link of links) {
      ...
      if (!new RegExp(`/jobs/view/${jobId}(?:/|\\?|$)`, 'i').test(href))
        continue;
  ...
  function titleFromPage(jobId, company) {
    ...
    const root = getJobDetailRoot();
    const selectedTitle = firstText$1(root, SELECTORS.title);
  ...
  function locationFromPage$1() {
    const root = getJobDetailRoot();
    const selected =
      firstText$1(root, SELECTORS.location) ||
      firstText$1(document, SELECTORS.location);
    if (selected) return selected.split(/\s*[·•]\s*/)[0] || void 0;
    const metadata = Array.from(root.querySelectorAll('p'))
      .map((element) => cleanText$4(element.textContent))
      .find(
        (text) =>
          /\s*[·•]\s*/.test(text) && /\b(?:ago|applicants?)\b/i.test(text),
      );
    return (
      (metadata == null ? void 0 : metadata.split(/\s*[·•]\s*/)[0]) || void 0
    );
  }
  function datePostedFromPage() {
    ...
    if (timeEl) {
      ...
      if (dt) return cleanText$4(dt);
      const text = cleanText$4(timeEl.textContent);
      if (text) return text;
    }
    const primaryDesc =
      firstText$1(root, [
      ...
      firstText$1(document, [
        '.job-details-jobs-unified-top-card__primary-description-container',
        '.jobs-unified-top-card__primary-description-container',
      ]);
    if (primaryDesc) {
      const segments = primaryDesc
        .split(/\s*[·•]\s*/)
        .map((s) => cleanText$4(s))
        .filter(Boolean);
      for (const seg of segments) {
        if (
          /\b(\d+\s*\+?\s*(?:minute|hour|day|week|month|year)s?\s+ago|today|yesterday|just\s+now|reposted)\b/i.test(
            seg,
          )
        ) {
          return seg;
  ...
  class LinkedInAdapter {
    ...
    readJob(url) {
      ...
      const root = getJobDetailRoot();
      const company =
        firstText$1(root, SELECTORS.company) ||
        firstText$1(document, SELECTORS.company) ||
        'Unknown company';
    ...
    getApplicationRoot() {
      if (
        ...
        isVisible$3(this.applicationRootCache)
      ) {
      ...
      const activeRoot = candidates.find(
        (candidate) =>
          ...
          this.isExplicitlyActiveModal(candidate),
      );
      if (activeRoot) {
      ...
      this.applicationRootCache =
        this.findApplicationRootFromHeading() ||
        this.findFullPageApplicationRoot();
      return this.applicationRootCache;
    }
    ...
    applicationFormDiagnostic() {
      ...
      const visibleCandidates = candidates.filter(
        (candidate) =>
          isVisible$3(candidate) && !this.hasHiddenModalAncestor(candidate),
      );
      ...
      const visibleFields = allFields.filter((field) => isVisible$3(field));
      const rootClasses =
        root && typeof root.className === 'string' ?
          root.className
            .trim()
            .split(/\s+/)
            .filter(Boolean)
      ...
      const rootDescription =
        ...
        : 'none';
      const fieldScopeDescription =
        root ? 'application root'
        : modalOutlet ? 'modal outlet fallback'
        : 'document fallback';
    ...
    getCurrentApplicationActionLabel() {
      ...
      return (
        cleanText$4(action.textContent || action.getAttribute('aria-label')) ||
        void 0
      );
    }
    ...
    async openApplication() {
      ...
      return {
        status: 'clicked',
        message:
          'LinkedIn Easy Apply click was dispatched; waiting for the application form.',
        url: window.location.href,
      };
    }
    async clickApplicationAction(action) {
      ...
      const documentSubmit =
        action === 'submit' ?
          findVisible(
            document,
            [...SELECTORS.submitAction, "button[type='submit']"],
            (candidate) =>
              /(?:submit|提交|应聘|申请)/i.test(
                cleanText$4(
                  candidate.textContent || candidate.getAttribute('aria-label'),
                ),
              ),
      ...
      const actionLabel2 =
        cleanText$4(button.textContent || button.getAttribute('aria-label')) ||
        void 0;
      try {
        button.scrollIntoView({ block: 'center', inline: 'nearest' });
      } catch {}
      ...
      const message =
        action === 'submit' ? 'LinkedIn application submitted.'
        : action === 'previous' ?
    ...
    findEasyApplyTrigger() {
      return findVisible(document, SELECTORS.easyApply, (element) => {
        ...
        const isEasyApplyText =
          /(?:easy\s*apply|简单申请|輕鬆應聘|轻松应聘|一键应聘|一键申请)/i.test(
            label,
          );
        const isEasyApplyUrl = /\/jobs\/view\/\d+\/apply(?:[/?#]|$)/i.test(
          href,
        );
        ...
        const hasEasyApplyTestHook = element.hasAttribute(
          'data-live-test-job-apply',
        );
        return (
          isEasyApplyText ||
          isEasyApplyUrl ||
          (isApplyButtonClass && hasEasyApplyTestHook)
        );
      });
    }
    isEasyApplyRoot(element) {
      ...
      const className =
        typeof element.className === 'string' ? element.className : '';
      if (/jobs-easy-apply-(?:modal|content|form)/i.test(className))
        return true;
      const label = cleanText$4(
      ...
      const hasApplicationField = Boolean(
        deepFirst(element, APPLICATION_FIELD_SELECTOR),
      );
      if (
        /(?:apply\s+to|申请(?:职位|工作)?|应聘)/i.test(label) &&
        hasApplicationField
      ) {
        return true;
      }
    ...
    applicationRootCandidates() {
      ...
      const candidates = [];
      const elements2 = deepElements(document);
    ...
    findApplicationRootFromHeading() {
      const heading = deepElements(document).find((element) => {
        if (!isVisible$3(element)) return false;
        const text = cleanText$4(element.textContent);
        return (
          /^apply\s+to\s+.+/i.test(text) ||
          /^申请(?:职位|工作)?\s*.+/.test(text)
        );
      });
      ...
      for (let depth = 0; candidate && depth < 9; depth += 1) {
        ...
        const shadowHost =
          (
            ...
            currentRoot.host instanceof HTMLElement
          ) ?
            currentRoot.host
          : null;
    ...
    hasApplicationAction(root) {
      return deepQueryAll(root, 'button, [role="button"]').some((button) => {
        ...
        return (
          isVisible$3(button) &&
          /(?:continue|next|review|submit|申请|提交|继续|下一步|审核|检查)/i.test(
            label,
          )
        );
      });
    ...
    isExplicitlyActiveModal(element) {
      ...
      return (
        ariaHidden === 'false' ||
        (ariaHidden === null && isVisible$3(container))
      );
    }
    ...
    hasCurrentJobReference(jobId) {
      ...
      if (titleLink)
        return jobPattern.test(titleLink.getAttribute('href') || '');
      const links = Array.from(
        document.querySelectorAll("a[href*='/jobs/view/']"),
      );
  ...
  function readLinkedInPage() {
    ...
    if (!jobId) {
      return {
        ...
        platform: 'linkedin',
        url,
        reason: 'The URL does not identify a LinkedIn job.',
      };
    }
    ...
    if (!job) {
      return {
        ...
        platform: 'linkedin',
        url,
        reason: 'The LinkedIn job title is not available yet.',
      };
    }
    const snapshot = {
      platform: 'linkedin',
  ...
  function isModalLike(element) {
    return element.matches(
      "dialog, [role='dialog'], [aria-modal='true'], [data-modal], [data-testid*='modal' i], [class*='modal' i]",
    );
  }
  ...
  function hasFormAction(scope) {
    return elementsInScope(scope).some((element) => {
      if (!element.matches(ACTION_SELECTOR) || !isVisibleElement(element))
        return false;
      return /submit|continue|next|review|apply|finish|save|send|提交|继续|下一步|审核|申请|完成|保存/i.test(
        actionLabel(element),
      );
    });
  }
  ...
  function readGenericAction(scope) {
    const actions = elementsInScope(scope)
      .filter(
        (element) =>
          element.matches(ACTION_SELECTOR) && isVisibleElement(element),
      )
      .map(actionLabel)
      .filter(Boolean);
    const submit = actions.find((label) =>
      /submit|apply|finish|send|提交|申请|完成/i.test(label),
    );
    if (submit) return { label: submit, action: 'submit' };
    const next = actions.find((label) =>
      /continue|next|review|save|继续|下一步|审核|保存/i.test(label),
    );
  ...
  function isLikelyLinkedInApplicationScope(scope) {
    ...
    return /easy apply|application questions|additional questions|review your application|contact information|work experience|resume|cover letter/i.test(
      text,
    );
  }
  function readLinkedInFormPage() {
    ...
    const applicationRoot =
      adapterRoot ||
      ((
        genericFallback instanceof HTMLElement &&
        isLikelyLinkedInApplicationScope(genericFallback)
      ) ?
        genericFallback
      : null);
    const genericAction =
      applicationRoot ? readGenericAction(applicationRoot) : {};
    const actionLabel2 =
      linkedinAdapter.getCurrentApplicationActionLabel() || genericAction.label;
    const actionKind =
      linkedinAdapter.getCurrentApplicationActionKind() || genericAction.action;
    ...
    if (
      inspection.kind === 'not_application_form' &&
      linkedinAdapter.isJobPageUrl(url)
    ) {
  ...
  const URL_JOB_PATTERN =
    /(?:^|[/_.-])(job|jobs|career|careers|position|positions|vacancy|vacancies|role|roles|jd|posting|postings|opening|openings|requisition)(?:[/_.?-]|$)/i;
  ...
  function elements(selectors, roots) {
    return roots.flatMap((root) =>
      Array.from(root.querySelectorAll(selectors)).filter(isVisible$2),
    );
  }
  function firstUsefulText(candidates, minLength = 2, maxLength = 180) {
    for (const candidate of candidates) {
      const text = cleanText$2(candidate.textContent);
  ...
  function labelledValue(labels, roots) {
    ...
    for (const element of elements(
      'dt, th, label, strong, b, span, div, p',
      roots,
    ).slice(0, 500)) {
      if (!labelPattern.test(cleanText$2(element.textContent))) continue;
      const sibling = element.nextElementSibling;
      const siblingText = cleanText$2(
        sibling == null ? void 0 : sibling.textContent,
      );
      if (siblingText && siblingText.length <= 180) return siblingText;
      const parentText = cleanText$2(
        (_a2 = element.parentElement) == null ? void 0 : _a2.textContent,
      );
      const match = parentText.match(
        new RegExp(`(?:${labels.join('|')})\\s*:\\s*([^|·•\\n]{2,120})`, 'i'),
      );
      if (match == null ? void 0 : match[1]) return cleanText$2(match[1]);
    }
    return '';
  }
  function companyFromBranding(roots) {
    for (const image of elements('img[alt]', roots)) {
      const alt = cleanText$2(image.getAttribute('alt'));
  ...
  function locationFromPage(roots) {
    ...
    return (
      semanticLocation ||
      labelledValue(['location', 'work location', 'location type'], roots)
    );
  }
  function descriptionFromPage(roots) {
    var _a2;
    const candidate = elements(DESCRIPTION_SELECTOR, roots)
      .map((element) => cleanText$2(element.textContent))
      .filter((text) => text.length >= 120)
      .sort((left, right) => right.length - left.length)[0];
    if (candidate) return truncate$1(candidate, 18e3);
    const heading = elements('h2, h3, h4', roots).find((element) =>
      JOB_HEADING_PATTERN.test(cleanText$2(element.textContent)),
    );
    const parentText = cleanText$2(
      (_a2 = heading == null ? void 0 : heading.parentElement) == null ?
        void 0
      : _a2.textContent,
    );
    return parentText.length >= 120 ? truncate$1(parentText, 18e3) : '';
  }
  function hasApplyAction(roots) {
    return elements(APPLY_SELECTOR, roots).some((element) => {
      const text = cleanText$2(
        element.getAttribute('aria-label') ||
          element.getAttribute('value') ||
          element.textContent,
      );
  ...
  function jobPostingFromStructuredData() {
    ...
    for (const script of scripts) {
      try {
        ...
        const address = firstLocation == null ? void 0 : firstLocation.address;
        const identifier = posting.identifier;
        const rawDesc = String(posting.description || '').replace(
          /<[^>]*>/g,
          ' ',
        );
        return {
          title: cleanText$2(String(posting.title || '')) || void 0,
          company:
            cleanText$2(
              String((organization == null ? void 0 : organization.name) || ''),
            ) || void 0,
          location:
            cleanText$2(
              [
                address == null ? void 0 : address.addressLocality,
                address == null ? void 0 : address.addressRegion,
                address == null ? void 0 : address.addressCountry,
              ]
                .filter((value) => typeof value === 'string')
                .join(', '),
            ) || void 0,
          description: cleanText$2(rawDesc) || void 0,
          externalId:
            cleanText$2(
              typeof identifier === 'string' ? identifier : (
                String((identifier == null ? void 0 : identifier.value) || '')
              ),
            ) || void 0,
          datePosted: cleanText$2(String(posting.datePosted || '')) || void 0,
        };
      } catch {}
    }
    return null;
  }
  function datePostedFromDom$1() {
    const timeEl = document.querySelector('time[datetime]');
    if (timeEl) {
      const dt = timeEl.getAttribute('datetime');
      if (dt) return cleanText$2(dt);
      const text = cleanText$2(timeEl.textContent);
      if (text) return text;
    }
    ...
    for (const el of candidates) {
      const text = cleanText$2(el.textContent);
      if (text.length > 60) continue;
      if (
        /\b(\d+\s*\+?\s*(?:minute|hour|day|week|month|year)s?\s+ago|today|yesterday|just\s+(?:now|posted)|reposted|posted\s+\d|\d+\s+days?)\b/i.test(
          text,
        )
      ) {
        return text;
  ...
  function readGenericJobPage() {
    const url = window.location.href;
    const roots = queryRoots();
    const structured = jobPostingFromStructuredData();
    const title =
      (structured == null ? void 0 : structured.title) ||
      firstUsefulText(elements(JOB_TITLE_SELECTOR, roots), 3, 180) ||
      '';
    const description =
      (structured == null ? void 0 : structured.description) ||
      descriptionFromPage(roots);
    const company =
      (structured == null ? void 0 : structured.company) ||
      labelledValue(
      ...
      '';
    const location =
      (structured == null ? void 0 : structured.location) ||
      locationFromPage(roots) ||
      '';
    const hasJobHeading = elements('h2, h3, h4', roots).some((element) =>
      JOB_HEADING_PATTERN.test(cleanText$2(element.textContent)),
    );
    ...
    const enoughEvidence =
      ...
      (Boolean(structured) ||
        (description.length >= 90 &&
          (applyAction || hasJobHeading || urlLooksLikeJob)) ||
    ...
    if (!enoughEvidence) {
      return {
        ...
        url,
        reason:
          'No job posting could be confirmed from the visible page content.',
      };
    }
    const snapshot = {
      platform: 'generic',
      externalId:
        (structured == null ? void 0 : structured.externalId) ||
        stableId$1(`${url}|${title}|${company}`),
      url,
      title,
      company: company || 'Unknown company',
      location: location || void 0,
      datePosted:
        (structured == null ? void 0 : structured.datePosted) ||
        datePostedFromDom$1(),
      description: description || void 0,
      technologies: extractTechnologyKeywords(description),
  ...
  function firstText(...selectors) {
    for (const selector of selectors) {
      ...
      if (el && isVisible$1(el)) {
        const text = cleanText$1(el.textContent);
        if (text) return text;
      }
    }
    return '';
      ...
      try {
        ...
        const locationStr =
          addressParts.length > 0 ?
            cleanText$1(addressParts.join(', '))
          : cleanText$1(
              typeof firstLocation === 'object' && firstLocation !== null ?
                String(firstLocation.name || '')
              : typeof location === 'string' ? location
              : '',
            );
        return {
          title: cleanText$1(String(posting.title || '')) || void 0,
          company:
            cleanText$1(
              String((organization == null ? void 0 : organization.name) || ''),
            ) || void 0,
          location: locationStr || void 0,
          description: cleanText$1(rawDesc) || void 0,
          externalId:
            cleanText$1(
              typeof identifier === 'string' ? identifier : (
                String((identifier == null ? void 0 : identifier.value) || '')
          ...
          datePosted: cleanText$1(String(posting.datePosted || '')) || void 0,
        };
      } catch {}
    }
    return null;
  ...
  function datePostedFromDom() {
    ...
    if (timeEl) {
      const dt = timeEl.getAttribute('datetime');
      if (dt) return cleanText$1(dt);
      const text = cleanText$1(timeEl.textContent);
      if (text) return text;
    }
    const selectors = [
    ...
    for (const el of candidates) {
      ...
      const text = cleanText$1(el.textContent);
      if (
        /\b(posted|today|yesterday|\d+\s*\+?\s*days?\s+ago|just\s+posted|recently\s+posted)\b/i.test(
          text,
        )
      ) {
        return text;
  ...
  function readIndeedJobPage() {
    ...
    if (!description) {
      ...
      if (descEl) {
        description = truncate(cleanText$1(descEl.textContent), 18e3);
      }
    }
    const externalId =
      (structured == null ? void 0 : structured.externalId) ||
    ...
    const datePosted =
      ...
      datePostedFromDom();
    const enoughEvidence =
      Boolean(title) && (Boolean(structured) || description.length >= 90);
    if (!enoughEvidence) {
      return {
        kind: 'unsupported_page',
        url,
        reason:
          'No Indeed job posting could be confirmed from the visible page content.',
      };
    }
    const snapshot = {
      platform: 'indeed',
      ...
      easyApply:
        Boolean(
          document.querySelector(
            "[id*='indeedApplyButton'], [class*='indeed-apply-button']",
          ),
  ...
  function classifyCurrentPage() {
    ...
    for (const pattern of KNOWN_JOB_HOSTS) {
      if (pattern.test(hostname2)) {
        reasons.push(`已知求职平台域名: ${hostname2}`);
        confidence += 5;
        autoQualify = true;
        break;
      }
    ...
    for (const script of ldScripts) {
      try {
        ...
        const types =
          Array.isArray(raw == null ? void 0 : raw['@type']) ?
            raw['@type']
          : [raw == null ? void 0 : raw['@type']];
        const graphItems = (raw == null ? void 0 : raw['@graph']) ?? [];
        const allTypes = [
          ...types,
    ...
    const headings = Array.from(document.querySelectorAll('h1, h2')).slice(
      0,
      10,
    );
  ...
  function isLinkedInHost(hostname2) {
    return hostname2 === 'linkedin.com' || hostname2.endsWith('.linkedin.com');
  }
  function isSeekHost$2(hostname2) {
    return (
      hostname2 === 'seek.com' ||
      hostname2.endsWith('.seek.com') ||
      hostname2 === 'seek.com.au' ||
      hostname2.endsWith('.seek.com.au')
    );
  }
  function isIndeedHost(hostname2) {
    return hostname2 === 'indeed.com' || /\.indeed\.com$/.test(hostname2);
  }
  ...
  function readCurrentPage() {
    ...
    if (isSeekHost$2(hostname2)) {
      const inspection = readSeekPage();
      return inspection.kind === 'job' ?
          inspection
        : fallbackToGenericJob(inspection);
    }
    if (isLinkedInHost(hostname2)) {
      const inspection = readLinkedInPage();
      return inspection.kind === 'job' ?
          inspection
        : fallbackToGenericJob(inspection);
    }
    if (isIndeedHost(hostname2)) {
  ...
  async function readCurrentPageWhenReady() {
    if (isLinkedInHost(window.location.hostname))
      return readLinkedInPageWhenReady();
  ...
  async function readLinkedInPageWhenReady() {
    ...
    for (let attempt = 0; attempt < 12; attempt += 1) {
      ...
      if (inspection.kind === 'job') {
        ...
        const routeChanged =
          !lastLinkedInRead || lastLinkedInRead.url !== observedUrl;
        const previousReadSignature =
          lastLinkedInRead ?
        ...
        if (
          ...
          (!lastLinkedInRead ||
            !routeChanged ||
            (contentChanged && previousSignature === signature2))
        ) {
          lastLinkedInRead = {
            url: observedUrl,
  ...
  function readCurrentForm() {
    if (isSeekHost$2(window.location.hostname)) return readSeekFormPage();
  ...
  function getCurrentFormScope() {
    if (isSeekHost$2(window.location.hostname))
      return findActiveFormScope() || document;
    if (isLinkedInHost(window.location.hostname))
      return linkedinAdapter.getApplicationRoot() || findActiveFormScope();
    return findActiveFormScope() || document;
  }
  ...
  function nodeHasFormSignal(node) {
    if (node instanceof Text) {
      ...
      return Boolean(
        parent == null ? void 0 : parent.closest(FORM_RELEVANT_SELECTOR),
      );
    }
    if (!(node instanceof Element)) return false;
    return (
      node.matches(FORM_RELEVANT_SELECTOR) ||
      Boolean(node.querySelector(FORM_RELEVANT_SELECTOR))
    );
  }
  function hasRelevantFormMutation(records) {
    return records.some((record) => {
      ...
      return (
        Array.from(record.addedNodes).some(nodeHasFormSignal) ||
  ...
  function watchFormScope(scope, readForm, initialForm) {
    var _a2, _b2;
    (_a2 = window.__jobbyFormObserverCleanup) == null ?
      void 0
    : _a2.call(window);
    (_b2 = window.__jobbyFormDiscoveryCleanup) == null ?
      void 0
    : _b2.call(window);
    if (!scope) return;
    let timer;
    ...
    const observer = new MutationObserver((records) => {
      records.forEach((record) => {
        record.addedNodes.forEach((node) =>
          observeShadowRootsIn(node, observeRoot),
        );
      });
    ...
    schedule = () => {
      ...
      timer = window.setTimeout(() => {
        ...
        void chrome.runtime
          .sendMessage({ type: 'content.form-changed', form })
          .catch(() => void 0);
        if (form.kind !== 'application_form') {
          (_a3 = window.__jobbyFormObserverCleanup) == null ?
            void 0
          : _a3.call(window);
    ...
    window.__jobbyFormObserverCleanup = () => {
      observer.disconnect();
      parentObserver == null ? void 0 : parentObserver.disconnect();
  ...
  function startFormDiscovery(readForm) {
    var _a2;
    (_a2 = window.__jobbyFormDiscoveryCleanup) == null ?
      void 0
    : _a2.call(window);
    const observedRoots = /* @__PURE__ */ new WeakSet();
    let timer;
    const discovery = new MutationObserver((records) => {
      ...
      if (
        !records.some((record) =>
          Array.from(record.addedNodes).some(hasDiscoverySignal),
        )
      )
        return;
      if (timer !== void 0) window.clearTimeout(timer);
  ...
  function hasDiscoverySignal(node) {
    if (!(node instanceof Element)) return false;
    return (
      node.matches(DISCOVERY_SELECTOR) ||
      Boolean(node.querySelector(DISCOVERY_SELECTOR))
    );
  }
  ...
  function labelFor(element, scope) {
    ...
    const legend = cleanText(
      (
        (_c =
          (_b2 = element.closest('fieldset')) == null ?
            void 0
          : _b2.querySelector('legend')) == null
      ) ?
        void 0
      : _c.textContent,
    );
  ...
  function findFormElement(target, scope) {
    ...
    if (target.name) {
      const elements2 = controls.filter(
        (element) => element.getAttribute('name') === target.name,
      );
      const visibleElements = elements2.filter((element) => isVisible(element));
      return (
  ...
  function fileUploadTrigger(input, scope) {
    ...
    return (
      (input.id ?
        queryScope.querySelector(`label[for='${CSS.escape(input.id)}']`)
      : null) ||
      (input.id ?
        queryScope.querySelector(`[aria-controls='${CSS.escape(input.id)}']`)
      : null) ||
      input
    );
  }
  ...
  function setValue(element, value) {
    ...
    const setter =
      (_a2 = Object.getOwnPropertyDescriptor(prototype, 'value')) == null ?
        void 0
      : _a2.set;
  ...
  function uploadFormFile(instruction, scope = document) {
    var _a2, _b2;
    if (!scope)
      return {
        commandId: instruction.commandId,
    ...
    try {
      const file = new File([bytes.slice().buffer], instruction.filename, {
    ...
    const selected = (_b2 = input.files) == null ? void 0 : _b2[0];
    if (
      !selected ||
  ...
  function matchesTarget(element, instruction, scope) {
    ...
    return (
      ...
      (normLabel === targetNorm ||
        (normLabel.length > 3 &&
          targetNorm.length > 3 &&
          (normLabel.includes(targetNorm) || targetNorm.includes(normLabel))))
    );
  }
  ...
  function clickRadioOption(element, scope) {
    ...
    const explicitLabel =
      element.id ?
        queryScope.querySelector(`label[for='${CSS.escape(element.id)}']`)
      : null;
    const target =
      explicitLabel || element.closest("label, [role='radio']") || element;
    clickControl(target);
  }
  ...
  function fillFormField(instruction, scope = document) {
    ...
    if (instruction.target.type === 'file') {
      ...
      if (
        ...
        const selection = selectExistingDocument(
          input,
          instruction.value,
          scope,
        );
    ...
    if (type === 'checkbox') {
      ...
      const label =
        checkbox.id ?
          queryScope.querySelector(`label[for='${CSS.escape(checkbox.id)}']`)
        : null;
      clickControl(
        label || checkbox.closest("label, [role='checkbox']") || checkbox,
      );
      if (checkbox.checked !== instruction.value) {
  ...
  async function handleContentCommand(message) {
    if (isInspectCommand(message))
      return {
        inspection: pageInspectionSchema.parse(
    ...
    if (isEditFormFieldCommand(message)) {
      ...
      return {
        fillResult: fillFormFieldValue(target, value, getCurrentFormScope()),
      };
    }
    if (isOpenLinkedInApplicationCommand(message))
      return { application: await linkedinAdapter.openApplication() };
    if (isLinkedInApplicationActionCommand(message)) {
      const action = linkedinApplicationActionSchema.parse(message.action);
      return {
        application:
          isSeekHost(window.location.hostname) ?
  ...
  function isInspectFormCommand(message) {
    return (
      ...
      message !== null &&
      message.type === 'content.inspect-form'
    );
  }
  function isFillFieldCommand(message) {
    return (
      typeof message === 'object' &&
      message !== null &&
      message.type === 'content.fill-field'
    );
  }
  function isFocusFormFieldCommand(message) {
    return (
      typeof message === 'object' &&
      message !== null &&
      message.type === 'content.focus-form-field'
    );
  }
  function isEditFormFieldCommand(message) {
    return (
      typeof message === 'object' &&
      message !== null &&
      message.type === 'content.edit-form-field'
    );
  }
  function isUploadFileCommand(message) {
    return (
      typeof message === 'object' &&
      message !== null &&
      message.type === 'content.upload-file'
    );
  }
  ...
  function isOpenLinkedInApplicationCommand(message) {
    return (
      typeof message === 'object' &&
      message !== null &&
      message.type === 'content.linkedin.open-application'
    );
  }
  function isLinkedInApplicationActionCommand(message) {
    return (
      typeof message === 'object' &&
      message !== null &&
      message.type === 'content.linkedin.application-action'
    );
  }
  if (window.__jobbyContentMessageListener) {
    chrome.runtime.onMessage.removeListener(
      window.__jobbyContentMessageListener,
    );
  }
