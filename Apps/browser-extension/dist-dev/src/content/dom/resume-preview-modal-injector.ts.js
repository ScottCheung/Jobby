import __vite__cjsImport0_react from "/vendor/.vite-deps-react.js__v--f5b0ea50.js"; const createElement = __vite__cjsImport0_react["createElement"];
import __vite__cjsImport1_reactDom_client from "/vendor/.vite-deps-react-dom_client.js__v--81f255ab.js"; const createRoot = __vite__cjsImport1_reactDom_client["createRoot"];
import { formatResumeFilename } from "/vendor/fs-Users-xianzhezhang-Projects-Jobby-Jobby-packages-ui-src-components-UI-Resume-index.ts.js";
import { Input } from "/vendor/fs-Users-xianzhezhang-Projects-Jobby-Jobby-packages-ui-src-components-UI-input-index.tsx.js";
import { formatRelativeTime } from "/src/shared/utils/date-formatter.ts.js";
...
let activeLibraryInputRoot = null;
let activeLibraryThemeListener = null;
...
function applyLibraryTheme(container) {
  const updateTheme = (stored) => {
    const color = typeof stored["auto-job-ui-theme-color"] === "string" ? stored["auto-job-ui-theme-color"] : "green";
    const storedMode = stored["auto-job-ui-theme"];
    const isDark = storedMode === "dark" || storedMode !== "light" && window.matchMedia("(prefers-color-scheme: dark)").matches;
  ...
  chrome.storage.local.get(
    ...
    (stored) => updateTheme(stored || {})
  );
  activeLibraryThemeListener = (changes, area) => {
    if (area === "local" && (changes["auto-job-ui-theme-color"] || changes["auto-job-ui-theme"])) {
      chrome.storage.local.get(
        ["auto-job-ui-theme-color", "auto-job-ui-theme"],
        (stored) => updateTheme(stored || {})
      );
...
export function showInPageResumeLibraryModal({
  ...
  const nativeSearch = shadow.querySelector("input");
  ...
  const updateList = () => {
    ...
    let visible = 0;
    shadow.querySelectorAll(".resume-card").forEach((card) => {
      const isHidden = Boolean(
        company && card.dataset.company !== company || query && !card.dataset.search?.includes(query)
      );
  ...
  shadow.querySelectorAll(".actions button[data-document]").forEach(
    (button) => button.addEventListener("click", () => {
      const item = button.closest(".resume-card");
      const documentType = button.dataset.document;
      if (!item?.dataset.id || documentType !== "resume" && documentType !== "cover_letter")
        return;
      shadow.querySelectorAll(".resume-card.selected").forEach((card) => card.classList.remove("selected"));
  ...
  shadow.querySelectorAll(".actions button[data-action=delete]").forEach(
    (button) => button.addEventListener("click", () => {
      const item = button.closest(".resume-card");
      if (!item?.dataset.id) return;
      if (!window.confirm("Delete this tailored resume? This cannot be undone."))
        return;
