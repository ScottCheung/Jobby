import { inspectVisibleFormFields, isVisibleElement } from "/src/content/dom/form-inspector.ts.js";
...
function hasFormAction(scope) {
  const actions = Array.from(scope.querySelectorAll(ACTION_SELECTOR));
...
export function findActiveFormScope(root = document) {
  const allCandidates = Array.from(root.querySelectorAll(CANDIDATE_SELECTOR));
  const candidates = allCandidates.filter((el) => isVisibleElement(el) && (el.matches("form, dialog, [role='dialog'], [aria-modal='true']") || Boolean(el.querySelector(CONTROL_SIGNAL_SELECTOR)))).slice(-20);
  let best = null;
  let bestScore = -1;
...
export function readGenericAction(scope) {
  let actions = Array.from(scope.querySelectorAll(ACTION_SELECTOR)).filter((element) => isVisibleElement(element)).map(actionLabel).filter(Boolean);
  let submit = actions.find((label) => SUBMIT_ACTION_REGEX.test(label));
  if (submit) return { label: submit, action: "submit" };
  if (scope !== document) {
    const docActions = Array.from(document.querySelectorAll(ACTION_SELECTOR)).filter((element) => isVisibleElement(element)).map(actionLabel).filter(Boolean);
...
export function hasGenericBackAction(scope) {
  const check = (s) => Array.from(s.querySelectorAll(ACTION_SELECTOR)).some((element) => {
    if (!isVisibleElement(element)) return false;
