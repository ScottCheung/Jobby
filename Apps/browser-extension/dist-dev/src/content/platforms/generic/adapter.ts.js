import { elementsInScope, isVisibleElement } from "/src/content/dom/form-inspector.ts.js";
...
function labelFor(element) {
  return cleanText(
    element.getAttribute("aria-label") || element.getAttribute("value") || element.getAttribute("title") || element.textContent
  );
}
function matchesAction(label, action) {
  ...
  return /(?:continue|next|review|proceed|save|继续|下一步|审核|保存)/i.test(label) && !/(?:submit|apply|send|finish|complete|confirm|提交|申请|发送|完成|确认)/i.test(label);
}
export function getGenericApplicationAction(action) {
  const scope = findActiveFormScope() || document;
  const inScope = elementsInScope(scope).find(
    (element) => element.matches(ACTION_SELECTOR) && isVisibleElement(element) && isEnabled(element) && matchesAction(labelFor(element), action)
  );
  if (inScope) return inScope;
  if (scope !== document) {
    return elementsInScope(document).find(
      (element) => element.matches(ACTION_SELECTOR) && isVisibleElement(element) && isEnabled(element) && matchesAction(labelFor(element), action)
    ) || null;
  }
  return null;
}
export async function clickGenericApplicationAction(action) {
  const button = getGenericApplicationAction(action);
  if (!button) {
    return {
      status: "unavailable",
      message: action === "previous" ? "The Back action is not available on this page." : action === "next" ? "The Continue action is not available on this page." : "The Submit action is not available on this page.",
  ...
  button.click();
  const formElement = button.closest("form") || button.form;
  if (formElement && action === "submit") {
    window.setTimeout(() => {
      try {
        if (typeof formElement.requestSubmit === "function") {
          formElement.requestSubmit(button instanceof HTMLInputElement || button instanceof HTMLButtonElement ? button : void 0);
        }
      } catch {
      }
    }, 100);
  ...
  return {
    status: "clicked",
    message: action === "previous" ? "Application moved to the previous step." : action === "next" ? "Application continued to the next step." : "Application submitted.",
    url: window.location.href,
...
export function findGenericApplyTrigger() {
  return elementsInScope(document).find((element) => {
...
export async function openGenericApplication() {
