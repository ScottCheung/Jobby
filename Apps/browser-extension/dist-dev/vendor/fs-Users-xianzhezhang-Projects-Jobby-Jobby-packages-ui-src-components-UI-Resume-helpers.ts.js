export function resumeFullName(data) {
  const basics = data.basics ?? {};
  return [basics.first_name, basics.middle_name, basics.last_name].filter(Boolean).join(" ") || "Resume";
}
export function resumeDateRange(start, end) {
  return [start, end].filter(Boolean).join(" - ");
}
export function resumeContactItems(data) {
  const basics = data.basics ?? {};
  const items = [];
  if (basics.email) {
    items.push({
  ...
  if (basics.linkedin_id) {
    const handle = basics.linkedin_id.replace(/^https?:\/\/(www\.)?linkedin\.com\/in\//i, "").replace(/^\/+|\/+$/g, "");
...
export function formatResumeFilename(data, company, jobTitle) {
  const basics = data?.basics || {};
  const fullName = [basics.first_name, basics.middle_name, basics.last_name].filter(Boolean).join(" ") || "Resume";
  const cleanName = fullName.replace(/[\\/:*?"<>|]/g, "").replace(/\s+/g, " ").trim();
  const cleanCompany = (company || "").replace(/[\\/:*?"<>|]/g, "").replace(/\s+/g, " ").trim();
  const cleanTitle = (jobTitle || "").replace(/[\\/:*?"<>|]/g, "").replace(/\s+/g, " ").trim();
  const parts = [cleanName, "CV", cleanCompany, cleanTitle].filter(Boolean);
  return `${parts.join("_")}.pdf`;
}
export function formatCoverLetterFilename(data, company, jobTitle) {
  const basics = data?.basics || {};
  const fullName = [basics.first_name, basics.middle_name, basics.last_name].filter(Boolean).join(" ") || "Candidate";
  const cleanName = fullName.replace(/[\\/:*?"<>|]/g, "").replace(/\s+/g, " ").trim();
  const cleanCompany = (company || "").replace(/[\\/:*?"<>|]/g, "").replace(/\s+/g, " ").trim();
  const cleanTitle = (jobTitle || "").replace(/[\\/:*?"<>|]/g, "").replace(/\s+/g, " ").trim();
...
export function templateCssVariables(config) {
  const px = (points) => `${points * config.paper.cssPixelsPerPoint}px`;
  return {
...
export function formatResumeAsPlainText(resume, competencies) {
  const parts = [];
  const basics = resume.basics;
  if (basics) {
    const name = [basics.first_name, basics.middle_name, basics.last_name].filter(Boolean).join(" ");
    if (name) parts.push(name);
    if (basics.headline) parts.push(basics.headline);
    const locationStr = basics.location ? [basics.location.city, basics.location.state, basics.location.country].filter(Boolean).join(", ") : "";
    const contacts = [
      basics.email,
      basics.phone,
      locationStr,
...
${allCompetencies.join(" • ")}`);
  ...
  if (resume.experience && resume.experience.length > 0) {
    const expStrings = resume.experience.map((exp) => {
      ...
      const techs = exp.technologies && exp.technologies.length > 0 ? `Technologies: ${exp.technologies.join(", ")}` : "";
...
${expStrings.join("\n\n")}`);
  ...
  if (resume.projects && resume.projects.length > 0) {
    const projStrings = resume.projects.map((proj) => {
      ...
      const techs = proj.technologies && proj.technologies.length > 0 ? `Technologies: ${proj.technologies.join(", ")}` : "";
      return [top, bullets, techs].filter(Boolean).join("\n");
    });
    parts.push(`
PROJECTS
...
${eduStrings.join("\n\n")}`);
  ...
  if (resume.certifications && resume.certifications.length > 0) {
    const certStrings = resume.certifications.flatMap(
      (group) => (group.certifications || []).map(
        (c) => [c.name, c.issuer, c.issue_date || c.date].filter(Boolean).join(" - ")
      )
    );
...
${certStrings.join("\n")}`);
  ...
  if (resume.languages && resume.languages.length > 0) {
    const langStrings = resume.languages.map((l) => [l.name, l.proficiency].filter(Boolean).join(" - ")).filter(Boolean);
...
export const defaultMasterResumeData = {
  basics: {
    first_name: "Scott",
    last_name: "Zhang",
  ...
  summary: "Pragmatic software engineer with strong experience delivering high-performance full-stack web applications and scalable cloud services.",
  core_competencies: [
