      ...
      <div className="diagnostics-content">
        ...
        <ol className="log-list">
          {diagnostics.slice().reverse().map((entry, idx) => {
            const badgeClass = LEVEL_BADGES[entry.level] || LEVEL_BADGES.debug;
            return (
              <li key={idx} className={`log-entry level-${entry.level} !grid gap-1`}>
                <div className="flex items-center gap-1.5 flex-wrap">
                  <span className={`px-1 rounded-[3px] text-[8px] font-extrabold uppercase leading-normal border ${badgeClass}`}>
                    {entry.level}
                  </span>
                  <span className="text-[9px] text-muted-foreground/80 font-mono">
                    {new Date(entry.timestamp).toLocaleTimeString()}
                  </span>
                  <span className="text-[9px] font-bold text-foreground/80 bg-muted/40 px-1 rounded truncate max-w-[120px]" title={entry.scope}>
                    {entry.scope}
                  </span>
                </div>
                <span className="text-[11px] text-foreground/90 leading-normal">{entry.message}</span>
              </li>
            );
          })}
        </ol>
