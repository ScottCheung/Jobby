...
function createPagePdfBlobUrl(pdfDataUrl: string): string | null {
  ...
  const onResponse = (event: Event) => {
    ...
    const detail = event.detail as { requestId?: unknown; ok?: unknown; blobUrl?: unknown };
...
export function showInPageResumeLibraryModal({
  ...
  const companies = Array.from(new Set(resumes.map((item) => item.company?.trim()).filter(Boolean))) as string[];
  ...
  const list = shadow.querySelector('.list') as HTMLDivElement;
  const updateList = () => {
    const query = search.value.trim().toLocaleLowerCase();
    const company = companyFilter.value;
    list.querySelectorAll<HTMLButtonElement>('.resume-card').forEach((card) => {
  ...
  shadow.querySelectorAll<HTMLButtonElement>('.actions button').forEach((button) => button.addEventListener('click', () => {
    ...
    // cannot be instantiated there. The side panel owns PDF generation.
    closeInPageResumeLibraryModal();
