...
def set_runtime_filter_values(sort_by_value: str, date_posted_value: str) -> None:
    search_config.sort_by = sort_by_value
    search_config.date_posted = date_posted_value
...
def set_search_location() -> None:
    ...
    '''
    search_location = search_config.search_location
...
def apply_filters(show_inpage_overlay, retry_count: int = 0) -> None:
    ...
    try:
        ...
        bot_status(f'Applying sort "{search_config.sort_by}" and date "{search_config.date_posted}"...')
        wait_span_click(_driver, search_config.sort_by)
        wait_span_click(_driver, search_config.date_posted)
        _wait_between(recommended_wait)

        bot_status("Applying experience and company filters...")
        multi_sel_noWait(_driver, search_config.experience_level)
        multi_sel_noWait(_driver, search_config.companies, _actions)
        if search_config.experience_level or search_config.companies:
            _wait_between(recommended_wait)

        bot_status("Applying job type and workplace filters...")
        multi_sel_noWait(_driver, search_config.job_type)
        multi_sel_noWait(_driver, search_config.on_site)
        ...
        multi_sel_noWait(_driver, search_config.location)
        multi_sel_noWait(_driver, search_config.industry)
        if search_config.location or search_config.industry:
            _wait_between(recommended_wait)

        bot_status("Applying function and title filters...")
        multi_sel_noWait(_driver, search_config.job_function)
        multi_sel_noWait(_driver, search_config.job_titles)
        ...
        wait_span_click(_driver, search_config.salary)
        _wait_between(recommended_wait)

        multi_sel_noWait(_driver, search_config.benefits)
        multi_sel_noWait(_driver, search_config.commitments)
